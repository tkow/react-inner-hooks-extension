...
type ReturnTypeStateTuple<State> = [State | undefined, (current: State | ((arg?: State) => State)) => void]
...
export function useStateFactory<State extends object = Record<string, any>>(
  ...
  const usePartialState = <StateName extends keyof State>(
    stateName: StateName
  ): ReturnTypeStateTuple<State[StateName]> => {
      ...
      setState((state) => {
        const newPartialState =
          typeof value === 'function'
            ? (value as (arg?: State[StateName]) => State[StateName])(state[stateName])
            : value
